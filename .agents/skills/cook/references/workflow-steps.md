...
**Task Tool Fallba** `TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList` are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking. All workflow steps remain functional without Task tools.
...
- **Compile che** run the relevant project compile/type-check command
- **Pattern verify:** confirm the new code matches adjacent conventions
- **Import che** confirm no circular dependency or dead import was added
...
After the subagent returns, log only — never re-run or blo
...
**Fallba** If `ck` is not available, edit plan.md directly —
