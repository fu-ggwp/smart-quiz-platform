...
```
/vdt:cook <natural language task OR plan path>
```
...
**Example:**
```
/vdt:cook "Add user authentication to the app" --fast
/vdt:cook path/to/plan.md --auto
/vdt:cook "Refactor auth middleware" --tdd
```
...
**Claude Tasks:** Utilize `TaskCreate`, `TaskUpdate`, `TaskGet`, `TaskList` during implementation step. **Fallbavdt:** These are CLI-only tools — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking instead.
...
- **Finalize (MANDATORY - never skip):**
  1. **Activate `/vdt:project-management` skill (MANDATORY)** → run full plan sync-back across ALL `phase-XX-*.md` (not only current phase), update `plan.md` status/progress, hydrate Claude Tasks, generate progress report
  ...
  5. Run `/vdt:journal` to write a concise technical journal entry upon completion
...
| Scout | `vdt:scout` | Optional in code |
| Plan | `planner` | Optional in code |
| UI Work | `ui-ux-designer` | If frontend work |
| Testing | `tester`, `debugger` | **MUST** spawn |
| Review | `code-reviewer` | **MUST** spawn |
| Finalize | `/vdt:project-management` skill + `docs-manager`, `git-manager` subagents | **MUST** invoke all |
...
**Typically follows:** `/vdt:plan` (execute a plan), `/vdt:brainstorm` (implement agreed solution)
**Typically precedes:** `/vdt:code-review` (review after implementation), `/vdt:test` (validate changes)
**Related:** `/vdt:fix` (alternative for bug fixes), `/vdt:plan` (create plan before cooking)
