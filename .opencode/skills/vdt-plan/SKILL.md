...
6. **Red Team Review** → Run `/vdt:plan red-team {plan-path}` (hard/deep/parallel/two modes)
7. **Post-Plan Validation** → Run `/vdt:plan validate {plan-path}` (hard/deep/parallel/two modes)
...
10. **Journal** → Run `/vdt:journal` to write a concise technical journal entry upon completion

### Whole-Plan Consistency Gate

This gate is mandatory after `/vdt:plan validate` or `/vdt:plan red-team` edits any plan file.
Load: `references/verification-roles.md` → "Whole-Plan Consistency Sweep".

Before recommending `/vdt:cook`, re-read `plan.md` and every `phase-*.md` file. Search all plan files for stale terms, rejected assumptions, renamed APIs/files/fields, superseded decisions, and duplicate embedded drafts/contracts. Reconcile contradictions across the entire plan, not only the edited phase.
...
**Fallbavdt:** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for tracking. Plan files remain the source of truth; hydration is an optimization, not a requirement.
...
|------------|-----------|---------|
| `/vdt:plan archive` | `references/archive-workflow.md` | Archive plans + write journal entries |
| `/vdt:plan red-team` | `references/red-team-workflow.md` | Adversarial plan review with hostile reviewers |
| `/vdt:plan validate` | `references/validate-workflow.md` | Validate plan with critical questions interview |
...
|--------|----------------|-----|
| `/vdt:plan validate` | Plan is moderate-to-complex; user wants critical-questions interview before implementation | Cheapest gate — surfaces unspecified assumptions, missing acceptance criteria, hand-wavy phases |
| `/vdt:plan red-team` | Plan touches security, auth, payments, data integrity, public APIs, infra, or has high blast radius | Adversarial reviewers stress-test the plan for failure modes, attack vectors, and missing edge cases |
| `/vdt:cook <plan-path>` | Plan is small / well-understood / low-risk and user wants to start implementation | Skip extra gates; go straight to implementation |
...
- Omit `/vdt:plan red-team` from the offered options when mode is `--hard`, `--deep`, `--parallel`, or `--two` (Step 6 already ran adversarial review).
- Omit `/vdt:plan validate` from the offered options when mode is `--deep` (Step 7 already ran validation).
- If both gates already ran, the Post-Plan Handoff still fires but offers only `/vdt:cook <plan-path>` and `End session`.
...
**Typically follows:** `/vdt:brainstorm` (after exploring options), `/vdt:scout` (after codebase discovery)
**May precede:** `/vdt:cook` after user approval (otherwise stop with plan path and next-step options)
**Related:** `/vdt:brainstorm` (explore before planning), `/vdt:cook` (execute after planning)
