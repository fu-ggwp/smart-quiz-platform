...
import QuizCard from "@/app/learner/study-sets/[id]/quiz/_components/quiz-card";
import QuestionMap from "@/app/learner/study-sets/[id]/quiz/_components/question-map";
import ConfirmExitModal from "@/app/learner/study-sets/[id]/quiz/_components/confirm-exit-modal";
...
export default function PublicQuizPage() {
  ...
  const handleSubmitQuiz = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/study-sets/${studySetId}/quiz/result?sessionId=${sessionId}`);
    } catch (err) {
  ...
  if (error) {
    ...
    const exitUrl = role === "learner" ? "/learner/study-sets" : "/";
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6 text-center">
        <div className="max-w-md bg-card border border-border rounded-3xl p-8 shadow-sm space-y-6">
          ...
          <Button className="w-full rounded-xl py-2.5 font-semibold" onClick={() => router.push(exitUrl)}>
            Back to Study Sets
          </Button>
        </div>
      </div>
