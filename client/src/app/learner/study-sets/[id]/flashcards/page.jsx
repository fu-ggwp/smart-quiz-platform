...
import { ArrowLeft, ChevronLeft, ChevronRight, Sparkles, BookOpen, PartyPopper } from "lucide-react";
import { studySetsService } from "@/services/study-sets.service";
import { Button } from "@/components/ui/button";

// Import các components con đã tách nhỏ
...
export default function LearnerFlashcardsPage() {
  ...
  // States quản lý phiên học & tiến độ (Session)
  ...
  useEffect(() => {
    ...
    async function initSession() {
      ...
      try {
        ...
        // 3. Tìm session gần nhất để khôi phục hoặc tạo mới
        ...
        // 4. Nếu không tìm thấy session nào dang dở, bắt đầu session mới
        if (!activeSessionId) {
  ...
  // --- Tự động lưu vị trí thẻ đang học vào localStorage ---
  useEffect(() => {
  ...
  const saveProgress = async (qId, status) => {
    ...
    try {
      ...
      await studySetsService.submitAnswer(sessionId, {
        question_id: qId,
        selected_answer_option_ids: [], // Trống đối với chế độ Flashcard
        is_correct: status === "mastered", // Đúng/đạt nếu đánh dấu đã thuộc
        review_status: status // "marked_for_retry", "mastered", hoặc "unreviewed"
      });
    } catch (err) {
  ...
  const toggleDifficult = (qId) => {
    ...
    if (isDiff) {
      ...
      newMastered.delete(qId); // Đã thuộc thì không thể là từ khó
  ...
  const markMastered = (qId) => {
    ...
    if (isMast) {
      ...
      newMastered.add(qId);
      newDifficult.delete(qId); // Đã thuộc thì bỏ đánh dấu khó
      saveProgress(qId, "mastered");
      
      // Tự động chuyển qua thẻ tiếp theo sau 300ms nếu còn thẻ
  ...
  const handleRestart = async () => {
    try {
      setSavingProgress(true);
      // Đặt lại toàn bộ thẻ về trạng thái chưa học trong database
  ...
  // --- Trang Chúc Mừng khi học hết thẻ ---
  if (currentIndex === questions.length) {
    return (
      <main className="h-[calc(100vh-80px)] max-h-[calc(100vh-80px)] overflow-hidden bg-background text-foreground flex flex-col justify-between py-4 px-4 md:px-8">
        ...
        <section className="flex-1 max-w-md w-full mx-auto flex flex-col justify-center items-center text-center space-y-6 px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full">
            ...
            <Button onClick={() => router.push(`/learner/study-sets/${studySetId}/quiz`)} variant="outline" className="flex-1 rounded-xl font-semibold py-2.5">
              Practice Quiz
            </Button>
          </div>
        </section>
  ...
  return (
    <main className="h-[calc(100vh-80px)] max-h-[calc(100vh-80px)] overflow-hidden bg-background text-foreground flex flex-col justify-between py-4 px-4 md:px-8">
      ...
      {/* CORE DISPLAY (FLASHCARD & INTEGRATED CONTROLS) */}
      <section className="flex-1 max-w-2xl w-full mx-auto flex flex-col justify-center items-center py-2">
        ...
        {/* Component Thẻ Flashcard (đã được tích hợp nút Khó/Thuộc bên trong) */}
        <Flashcard 
